...
export default function RaipurDatabase() {
  ...
  return <div className="page-content">
    <section className="section-panel"><div className="section-panel-header"><div><h2>Bulk Paste Raipur Data</h2><p>Paste planning and vehicle fields together. Grouping uses Date + CFA + Loading; Loc and STO do not split a plan.</p></div></div><div className="section-panel-body">
      ...
      {preview && <ImportReadyBar result={preview} existingCount={rows.length} onImport={importPreview} />}
...
function ImportReadyBar({ result, onImport, existingCount }) {
  return <div className="import-ready-bar" role="status">
    <span><strong>Ready to import</strong> · Raw Rows: <b>{result.rawRowCount}</b> · Final Plans: <b>{result.finalRowCount}</b> · Existing: <b>{existingCount}</b></span>
