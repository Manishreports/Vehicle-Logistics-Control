...
export default function RaipurDatabase() {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => !search || Object.values(row).some((value) => String(value ?? '').toLowerCase().includes(search.toLowerCase()))).map((row) => ({ ...row, date: displayBusinessDate(row.date), vehicleIn: displayBusinessDate(row.vehicleIn), vehicleOut: displayBusinessDate(row.vehicleOut) })), [rows, search]);
...
function RaipurGroupedTable({ rows }) {
  ...
  return <div className="table-wrap"><table className="enterprise-table grouped-plan-table"><thead><tr>{COLUMNS.map((column) => <th key={column.key}>{column.label}</th>)}</tr></thead><tbody>{rows.flatMap((plan) => {
    ...
    return childRows.map((child, index) => <tr key={`${plan.groupKey}-${index}`} className={plan.conflicts?.length ? 'conflict-row' : ''}>
      {index === 0 && <><td rowSpan={planSpan}>{plan.serialNo}</td><td rowSpan={planSpan}>{displayBusinessDate(plan.date)}</td></>}
